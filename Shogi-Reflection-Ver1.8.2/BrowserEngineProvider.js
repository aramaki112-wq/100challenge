...
function verifiedManifestMetadata(manifest) {
  return Boolean(
    manifest?.available === true &&
    manifest?.engineVersion &&
    manifest?.commitHash &&
    manifest?.emscriptenVersion &&
    manifest?.jsSha256 &&
    manifest?.wasmSha256
  );
}

function yaneuraOuBrowserCapability(windowObject, manifest) {
  if (!verifiedManifestMetadata(manifest)) {
    return Object.freeze({ usable: false, reason: "YaneuraOu WASMのBuild/Hash metadataが未検証です。" });
  }
  if (!windowObject?.Worker) {
    return Object.freeze({ usable: false, reason: "このBrowserではWeb Workerを利用できません。" });
  }
  if (manifest.requiresCrossOriginIsolation === true && windowObject.crossOriginIsolated !== true) {
    return Object.freeze({ usable: false, reason: "YaneuraOu WASM Threadsに必要なcross-origin isolationが有効ではありません。" });
  }
  if (manifest.requiresThreads === true && typeof windowObject.SharedArrayBuffer !== "function") {
    return Object.freeze({ usable: false, reason: "YaneuraOu WASM Threadsに必要なSharedArrayBufferを利用できません。" });
  }
  return Object.freeze({ usable: true, reason: "" });
}
...
export async function resolveBrowserEngine(windowObject = window) {
  ...
  if (manifest) {
    const capability = yaneuraOuBrowserCapability(windowObject, manifest);
    ...
    return localFallback(windowObject, capability.reason);
  }
