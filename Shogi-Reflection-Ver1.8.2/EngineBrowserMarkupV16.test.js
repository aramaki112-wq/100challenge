...
test("STEP3にEngine解析・進捗・Cancel・候補領域がある",()=>{ for(const id of ["analyze-game","cancel-analysis","engine-analysis-status","engine-analysis-progress","engine-analysis-candidates"]) assert.match(html,new RegExp(`id="${id}"`)); });
test("Engine候補は自動登録せず明示ButtonでKeyPositionへ接続",()=>{ assert.match(html,/Engineは重要局面を自動確定しません/); assert.match(main,/data-engine-add-key-position/); });
