...
Bad Candidateは`bestMoveDifferenceCp`（Root Best Evaluationと実戦手後Actual Evaluationの差）をlegacy before/after deltaより優先する。閾値を単純に下げるのではなく、Real searchでfuture tactical loss等が評価へ入ることを前提とする。

追加回帰ケース: clear blunder / quiet blunder / tactical loss / mate miss / winning→equal / winning→losing / best-vs-actual priority / good unique-ish defense / no false forced five。

ReflectionLocalはmaterial中心Fallbackのため、これらの実用品質をReal YaneuraOuで証明したことにはならない。
