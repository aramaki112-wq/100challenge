...
### Official wasm_pre.js contract

Pinned V9.00の`wasm_pre.js`はModuleへmessage listener、`postMessage(command)`、command queue/retry、terminateを提供する。`usi_command` C exportをWorker bootstrapから直接叩くとThread起動待ち制御を迂回するため、Bootstrapはofficial Module message bridgeを使用するよう修正した。

### Browser Capability Gate

Real primary選択には最低限、verified Build metadata + JS/WASM SHA-256 + Worker + `crossOriginIsolated` + `SharedArrayBuffer`（Thread build時）を要求する。失敗時は明示ReflectionLocal fallback。Engine Nameは混同しない。

### Evaluation Graph

`AnalyzeGame`のall-ply timelineをGraphへ渡すが、GraphはNavigation/View層でありDomainへYaneuraOu/WASM概念を追加しない。詳細は`ENGINE_EVALUATION_GRAPH_DESIGN.md`。
