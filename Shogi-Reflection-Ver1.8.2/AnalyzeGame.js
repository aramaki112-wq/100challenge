...
export class AnalyzeGame {
  ...
  async execute({ gameId, history, playerSide, settings, onProgress = () => {}, signal = null } = {}) {
    ...
    const analyses = [];
    const maxAnalyzedPly = Math.min(history.maxMoveNumber, appliedSettings.maxPlies ?? history.maxMoveNumber);
    const totalPositions = maxAnalyzedPly + 1;

    for (let ply = 0; ply <= maxAnalyzedPly; ply += 1) {
      ...
      onProgress(Object.freeze({ completed: ply + 1, total: totalPositions, ply }));
    }

    // Ver.1.8.2: preserve every analyzed ply as a lightweight graph/navigation timeline.
    // Search trees are intentionally not persisted; only normalized evaluation and small metadata.
    const evaluationTimeline = Object.freeze(analyses.map((item) => Object.freeze({
      ply: item.ply,
      evaluation: item.evaluation,
      depth: item.raw.depth ?? null,
      nodes: item.raw.nodes ?? null,
      time: item.raw.time ?? item.raw.analysisTime ?? null,
      analysisTime: item.raw.analysisTime ?? null,
      bestMove: item.raw.bestMove ?? null
    })));

    const rows = [];
    for (let ply = 1; ply <= maxAnalyzedPly; ply += 1) {
      ...
      const candidates = (before.raw.candidateMoves ?? []).map((candidate) => Object.freeze({
        ...candidate,
        evaluation: candidate.evaluation
          ? normalizeEvaluation(candidate.evaluation, { sideToMove: before.positionSideToMove, viewerSide: playerSide })
          : null
      }));
      const bestMove = before.raw.bestMove ?? candidates[0]?.move ?? null;
      const evaluationDelta = calculateEvaluationDelta(before.evaluation, after.evaluation);
      const bestMoveDifferenceCp = evaluationDelta.kind === "CP_CHANGE" ? evaluationDelta.centipawns : null;
      const bestLine = candidates.find((candidate) => candidate.rank === 1) ?? candidates[0] ?? null;
      rows.push(Object.freeze({
        ...
        // evaluationBefore is the Engine best evaluation for the position before the actual move.
        // evaluationAfter is the Engine evaluation of the position reached by the actual move.
        // Keep legacy names and add explicit comparison names so persisted Ver.1.7/1.8 Baseline
        // readers remain compatible while the UI can explain Best vs Actual without ambiguity.
        evaluationBefore: before.evaluation,
        evaluationAfter: after.evaluation,
        bestEvaluation: before.evaluation,
        actualEvaluation: after.evaluation,
        evaluationDelta,
        bestMoveDifferenceCp,
        bestMovePv: Object.freeze((bestLine?.pv ?? []).slice(0, 6)),
    ...
    return Object.freeze({
      ...
      totalPositionsInGame: history.maxMoveNumber + 1,
      analysisTruncated: maxAnalyzedPly < history.maxMoveNumber,
      maxAnalyzedPly,
      rows: Object.freeze(rows),
      evaluationTimeline,
      primaryCandidates: selection.primaryCandidates,
      goodCandidates: selection.goodCandidates,
      badCandidates: selection.badCandidates,
