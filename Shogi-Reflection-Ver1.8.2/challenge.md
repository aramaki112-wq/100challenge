...
Evaluation GraphをNavigation Mapとして完成させ、Good/Bad Candidateと本人KeyPositionを同じ対局Timeline上で接続する。Engine情報はReferenceに留め、本人のFACT/INTERPRETATION/HYPOTHESISへ自動入力しない。

Formal Gateは通常Testから分離する。`npm test`が成功してもReal YaneuraOu Artifact/E2Eがなければ正式完成ではない。
