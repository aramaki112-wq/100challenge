...
test("KIF→Position History→ReflectionLocal fallback Evaluation→Normalization→Candidate Rankingを通す", async()=>{
  ...
  assert.ok(result.goodCandidates.length<=5);
  assert.ok(result.badCandidates.length<=5);
...
test("短い棋譜で合理的候補が3件未満なら水増ししない", async()=>{
  ...
  assert.ok(result.primaryCandidates.length<=Math.min(10,result.totalCandidates));
  assert.ok(result.goodCandidates.length<=5);
  assert.ok(result.badCandidates.length<=5);
  await engine.dispose();
});
