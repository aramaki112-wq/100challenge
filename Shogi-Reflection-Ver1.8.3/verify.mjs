...
const required = [
  ...
  ".github/workflows/build-yaneuraou-wasm.yml", "scripts/build-yaneuraou-wasm.sh", "scripts/hash-engine-assets.sh", "scripts/update-engine-build-metadata.mjs",
  "scripts/verify-yaneuraou-wasm.sh", "scripts/integrate-yaneuraou-build-artifact.sh", "scripts/finalize-yaneuraou-manifest.mjs",
  "scripts/update-engine-license-gate.mjs", "scripts/run-node-tests-with-evidence.sh", "requirements-real-engine.txt",
  "scripts/real-yaneuraou-artifact-gate.mjs", "scripts/formal-completion-gate.mjs", "YaneuraOuBuildBridgeV183.test.js",
  "ENGINE_BUILD_METADATA.json", "ENGINE_BUILD_RESULT.txt", "ENGINE_ASSET_SHA256SUMS.txt", "ENGINE_REAL_USI_RESULT.txt", "ENGINE_REAL_E2E_RESULT.txt",
  "YANEURAOU_WASM_BUILD_BRIDGE.md", "REAL_YANEURAOU_USI_RESULT.json", "REAL_YANEURAOU_E2E_RESULT.json", "REAL_YANEURAOU_BROWSER_E2E_RESULT.txt",
  "real_yaneuraou_usi_verify.py", "real_yaneuraou_browser_verify.py", "ENGINE_REAL_PERFORMANCE_RESULT.json", "ENGINE_LICENSE_GATE_RESULT.json",
...
check("package version", pkg.version === "1.8.3", pkg.version);
...
check("Real USI verifier script", pkg.scripts?.["test:real-yaneuraou-usi"] === "python3 real_yaneuraou_usi_verify.py");
check("Real application E2E verifier script", pkg.scripts?.["test:real-yaneuraou-browser"] === "python3 real_yaneuraou_browser_verify.py");
...
check("Build metadata does not fabricate measured values", buildMetadata.measured === false ? (buildMetadata.jsSha256 === null && buildMetadata.wasmSha256 === null && buildMetadata.workerSha256 === null) : Boolean(buildMetadata.jsSha256 && buildMetadata.wasmSha256 && buildMetadata.workerSha256));
...
check("Candidate Jump uses existing Replay + deterministic intentional board scroll", cjump.includes("replayController.jump") && cjump.includes("replayView.scrollIntoView") && cjump.includes('behavior: "auto"') && cjump.includes("ENGINE_CANDIDATE_JUMP") && !cjump.includes("navigateToStep"));
const graphJumpStart = main.indexOf('const replayMarker = target.closest?.("[data-engine-graph-replay-ply]")');
const graphJumpEnd = main.indexOf("return true;", graphJumpStart);
const graphJump = graphJumpStart >= 0 && graphJumpEnd >= 0 ? main.slice(graphJumpStart, graphJumpEnd) : "";
check("Graph Replay Jump uses deterministic intentional board scroll", graphJump.includes("replayController.jump") && graphJump.includes("replayView.scrollIntoView") && graphJump.includes('behavior: "auto"') && graphJump.includes("ENGINE_GRAPH_CANDIDATE_JUMP"));
...
check("Unverified WASM cannot be silently enabled", manifest.available === true ? Boolean(manifest.jsSha256 && manifest.wasmSha256 && manifest.emscriptenVersion) : (manifest.status === "NOT_BUILT_IN_CURRENT_VERIFICATION_ENVIRONMENT" && manifest.jsSha256 === null && manifest.wasmSha256 === null));
...
const report = [
  ...
  "Date: 2026-08-10",
  "Source of Truth: user-supplied Shogi-Reflection-Ver1.8.2-NOT-FORMAL-Integration-Candidate(1).zip",
  "",
  `Files: ${files.length}`,
