...
const POOL4_SHA = 'f8b82eef754d44488b093c9b688cb0ec43b40985162204c6682eebb963b9a013';
const STACK8_SHA = '5fcee573b4a0a898e747e303b6e003db8af99c901d63b15b88e88708057678d9';

test('A5-E5はPTHREAD_POOL_SIZEだけをA5-E2 baseline 1からcandidate 4へ変更する', () => {
  const poolPatch = path.join(HERE, 'ios-real-engine-pool4-stack8-gate/patches/yaneuraou-v9.00-pthread-pool-size-4.patch');
  const s = read(poolPatch);
  assert.equal(sha(poolPatch), POOL4_SHA);
  assert.match(s, /-\s*LDFLAGS \+= -s PTHREAD_POOL_SIZE=32/);
  assert.match(s, /\+\s*LDFLAGS \+= -s PTHREAD_POOL_SIZE=4/);
  const changed = s.split(/\r?\n/).filter(line => (/^[+-]/.test(line) && !/^(---|\+\+\+)/.test(line)));
  assert.equal(changed.length, 2);
  assert.ok(changed.every(line => line.includes('PTHREAD_POOL_SIZE')));
});

test('A5-E5はA5-E2のStack8 patchをそのまま保持する', () => {
...
test('A5-E5 workflowは別artifactを生成しFormal releaseを呼ばない', () => {
  const rootWorkflow = path.join(repoRoot, '.github/workflows/yaneuraou-ios-pool4-stack8-candidate.yml');
  const appWorkflow = path.join(HERE, '.github/workflows/yaneuraou-ios-pool4-stack8-candidate.yml');
  for (const p of [rootWorkflow, appWorkflow]) {
    const s = read(p);
    assert.match(s, /name: YaneuraOu iPhone Pool4 Stack8 Candidate/);
    assert.match(s, /Build iPhone Pool4 STACK_SIZE=8MiB candidate/);
    assert.match(s, /name: yaneuraou-ios-pool4-stack8-candidate/);
    assert.match(s, /IOS_POOL4_STACK8_CANDIDATE_METADATA\.json/);
    assert.match(s, /ios-real-engine-pool4-stack8-gate/);
...
test('A5-E5 build scriptはPool4だけを変更しStack8とInitial Memoryを固定する', () => {
  const s = read(path.join(HERE, 'ios-real-engine-pool4-stack8-gate/build-ios-pool4-stack8-candidate.sh'));
  assert.match(s, new RegExp(`EXPECTED_POOL_PATCH_SHA="${POOL4_SHA}"`));
  assert.match(s, new RegExp(`EXPECTED_STACK_PATCH_SHA="${STACK8_SHA}"`));
  assert.match(s, /PTHREAD_POOL_SIZE=4/);
  assert.match(s, /STACK_SIZE=8388608/);
  assert.match(s, /EM_INITIAL_MEMORY_SIZE=92274688/);
  assert.match(s, /INITIAL_MEMORY=92274688/);
  assert.match(s, /pthreadPoolSize!==4/);
...
test('A5-E5 resultは一条件変更・NOT_FORMALを明示する', () => {
  const result = JSON.parse(read(path.join(HERE, 'PWA_RUN_A5_E5_RESULT.json')));
  assert.equal(result.controlledChange.setting, 'PTHREAD_POOL_SIZE');
  assert.equal(result.controlledChange.baseline, 1);
  assert.equal(result.controlledChange.candidate, 4);
...
test('SMARTPHONE_SAFEのUSI ThreadsはA5-E5でも1のまま', () => {
