...
def build_html(seed: dict[str, str] | None = None, *, real_engine: bool = False) -> str:
    ...
    worker_source_json = json.dumps((ROOT / "ReflectionLocalEngineWorker.js").read_text(encoding="utf-8"), ensure_ascii=False) if real_engine else "null"
    sample_kif_b64 = base64.b64encode((ROOT / "samples/piyo_20260617_170236.kif").read_bytes()).decode("ascii")
    engine_bootstrap = f"""
  const localEngineWorkerSource = {worker_source_json};
  if (localEngineWorkerSource) {{
    window.SHOGI_REFLECTION_LOCAL_ENGINE_WORKER_URL = URL.createObjectURL(new Blob([localEngineWorkerSource], {{ type: 'text/javascript' }}));
  }} else {{
...
(() => {{
  ...
  const sampleKifBase64 = {json.dumps(sample_kif_b64)};
  const originalFetch = window.fetch?.bind(window);
  window.fetch = async (input, init) => {{
    const url = String(input?.url ?? input ?? '');
    if (url.endsWith('/samples/piyo_20260617_170236.kif') || url.endsWith('samples/piyo_20260617_170236.kif')) {{
      const binary = atob(sampleKifBase64);
...
{engine_bootstrap}  let clipboardText = '';
...
def run() -> int:
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(executable_path=os.environ.get("PLAYWRIGHT_EXECUTABLE_PATH") or ("/usr/bin/chromium" if Path("/usr/bin/chromium").exists() else None), headless=True, args=["--no-sandbox"] if (os.environ.get("PLAYWRIGHT_EXECUTABLE_PATH") or Path("/usr/bin/chromium").exists()) else [])
        ...
        checks.check("Ver1.8 title", "Ver.1.8" in page.title(), page.title())
        ...
        # Bundled sample KIF uses the same existing preview/import path.
        page.click("#load-sample-kif")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        sample_preview = page.locator("#kif-import-preview").inner_text()
        checks.check("Sample KIF Preview", "152" in sample_preview and "Lv18" in sample_preview, sample_preview[:160])
        ...
        # Register KIF -> Step2 and save game only. Use opening header for Ver.1.8 summary verification.
        ...
        checks.check("Engine status completed", "解析完了" in page.locator("#engine-analysis-status").inner_text())
        ...
        good_candidate_count = page.locator('[data-engine-candidate][data-candidate-group="GOOD"]').count()
        bad_candidate_count = page.locator('[data-engine-candidate][data-candidate-group="BAD"]').count()
        checks.check("Engine candidate displayed", candidate_count >= 1)
        checks.check("Good candidate max five", good_candidate_count <= 5, str(good_candidate_count))
        checks.check("Bad candidate max five", bad_candidate_count <= 5, str(bad_candidate_count))
        checks.check("Engine candidate aggregate max ten", candidate_count <= 10, str(candidate_count))
        ...
        checks.check("Analysis progress text recorded", "局面" in page.locator("#engine-analysis-progress-text").inner_text())
        checks.check("Evaluation Graph rendered", page.locator("#engine-evaluation-graph svg.engine-evaluation-graph").count() == 1)
        checks.check("Evaluation Graph has all-ply points", page.locator("#engine-evaluation-graph [data-engine-graph-ply]").count() >= 2)
        checks.check("Evaluation Graph has Candidate marker", page.locator("#engine-evaluation-graph [data-engine-graph-replay-ply]").count() >= 1)
        graph_ply = int(page.locator("#engine-evaluation-graph [data-engine-graph-replay-ply]").first.get_attribute("data-engine-graph-replay-ply"))
        page.locator("#engine-evaluation-graph [data-engine-graph-replay-ply]").first.click()
        page.wait_for_function("ply => document.querySelector('#replay-jump-number').value===String(ply)", arg=graph_ply)
        checks.check("Evaluation Graph Candidate -> Replay", page.locator("#replay-jump-number").input_value() == str(graph_ply))
        ...
        board_view = page.evaluate("""() => { const r=document.querySelector('.replay-board-shell').getBoundingClientRect(); const nav=document.querySelector('.step-navigation')?.getBoundingClientRect(); return {top:r.top,bottom:r.bottom,height:r.height,viewport:innerHeight,navBottom:nav?.bottom ?? 0,scrollY:scrollY}; }""")
        checks.check("Candidate Jump intentionally scrolls toward Board", abs(page.evaluate("window.scrollY") - candidate_jump_y) > 1, str((candidate_jump_y, page.evaluate("window.scrollY"))))
        checks.check("Candidate Jump Board visible", board_view["bottom"] > 0 and board_view["top"] < board_view["viewport"], str(board_view))
        checks.check("Candidate Jump Board not hidden by sticky header", board_view["top"] >= board_view["navBottom"] - 2, str(board_view))
        ...
        checks.check("Candidate add keeps Page scroll", abs(page.evaluate("window.scrollY") - add_y) <= 5, str((add_y, page.evaluate("window.scrollY"))))
        ...
        checks.check("Evaluation Graph KeyPosition marker appears", page.locator(f'#engine-evaluation-graph [data-engine-graph-key-position-ply="{candidate_ply}"]').count() == 1)
        page.locator(f'#engine-evaluation-graph [data-engine-graph-key-position-ply="{candidate_ply}"]').click()
        page.wait_for_function("document.querySelector('#step-menu').value === '4'")
        page.wait_for_function("document.activeElement?.getAttribute('data-field') === 'fact'")
        checks.check("Graph KeyPosition -> STEP4 exact Card", page.evaluate("ply => document.activeElement?.closest('[data-key-position]')?.querySelector('[data-field=moveNumber]')?.value === String(ply)", candidate_ply))
        ...
        # Long fixture verifies per-group maximums without forcing a minimum candidate count.
        long_page.evaluate("document.querySelector('#analyze-game').click()")
        long_page.wait_for_function("document.querySelector('#engine-analysis-status')?.dataset.status === 'COMPLETED'", timeout=20000)
        long_candidate_count = long_page.locator("[data-engine-candidate]").count()
        long_good_count = long_page.locator('[data-engine-candidate][data-candidate-group="GOOD"]').count()
        long_bad_count = long_page.locator('[data-engine-candidate][data-candidate-group="BAD"]').count()
        checks.check("Long game candidate aggregate max ten", long_candidate_count <= 10, str(long_candidate_count))
        checks.check("Long game Good max five", long_good_count <= 5, str(long_good_count))
        checks.check("Long game Bad max five", long_bad_count <= 5, str(long_bad_count))
    ...
    result_lines = [
        "Shogi Reflection Ver.1.8 Browser Verification",
        "==============================================",
