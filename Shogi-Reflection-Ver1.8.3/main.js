...
function currentKeyPositionsForGraph() {
  return formView.readInput().keyPositions ?? [];
}

function refreshEngineGraphKeyPositions() {
  engineAnalysisView.setKeyPositions(currentKeyPositionsForGraph());
}

function renderSavedAnalysis(gameId) {
  const latest = engineAnalysisRepository.findLatestByGameId(gameId);
  if (latest) engineAnalysisView.renderResult(latest, { keyPositions: currentKeyPositionsForGraph() });
...
document.getElementById("load-sample-kif").addEventListener("click", async () => {
  try {
    const response = await fetch("./samples/piyo_20260617_170236.kif", { cache: "no-store" });
    if (!response.ok) throw new Error(`サンプル棋譜を取得できませんでした: HTTP ${response.status}`);
    const bytes = await response.arrayBuffer();
    const file = new File([bytes], "piyo_20260617_170236.kif", { type: "application/octet-stream" });
    await previewKifFile(file);
  } catch (error) {
    kifImportView.showFeedback({ kind: "error", title: "サンプル棋譜を読み込めませんでした", message: String(error?.message ?? error) });
  }
});
...
function addCurrentReplayPositionToKeyPosition({ focus = true, showFormFeedback = true } = {}) {
  ...
  currentHasUnsavedChanges = true;
  refreshEngineGraphKeyPositions();
...
document.getElementById("add-key-position").addEventListener("click", () => { formView.addKeyPosition(); replayView.setKeyPositionCount(formView.getMeaningfulKeyPositionCount()); refreshEngineGraphKeyPositions(); currentHasUnsavedChanges = true; });
document.getElementById("key-position-list").addEventListener("click", (event) => {
  ...
  if (button) { formView.removeKeyPosition(button); replayView.setKeyPositionCount(formView.getMeaningfulKeyPositionCount()); refreshEngineGraphKeyPositions(); currentHasUnsavedChanges = true; }
});
document.getElementById("key-position-list").addEventListener("input", (event) => {
  // A manually edited move number is still the existing KeyPosition source of truth.
  // Reflect it on the graph immediately; do not create graph-owned KeyPosition state.
  if (event.target.closest?.('[data-field="moveNumber"]')) refreshEngineGraphKeyPositions();
});
...
document.getElementById("analyze-game").addEventListener("click", async () => {
  ...
  try {
    ...
    const appliedSettings = engineAnalysisSettings("SMARTPHONE_SAFE");
    ...
    engineAnalysisView.renderResult(result, { keyPositions: currentKeyPositionsForGraph() });
  } catch (error) {
...
document.getElementById("engine-analysis-candidates").addEventListener("click", (event) => {
  ...
  if (replayButton) {
    // Ver.1.8 formal exception to Replay Scroll Policy:
    // ordinary Replay navigation keeps pageScroll=NONE, but this explicit "局面を見る" action
  ...
  if (addButton) {
    try {
      // Candidate -> KeyPosition is a data-registration action, not a page-navigation action.
      // Updating STEP4 / graph DOM can trigger browser scroll anchoring because content above
      // the Replay board changes height. Preserve the current page viewport explicitly.
      const preservedScroll = { x: window.scrollX, y: window.scrollY };
      const restoreScroll = () => window.scrollTo({ left: preservedScroll.x, top: preservedScroll.y, behavior: "auto" });
      const rendered = replayController.jump(Number(addButton.dataset.engineAddKeyPosition));
      if (rendered) {
        addCurrentReplayPositionToKeyPosition({ focus: false, showFormFeedback: false });
        restoreScroll();
...
// Evaluation Graph Navigation. Candidate points reuse Replay; KeyPosition points open exact STEP4 card.
function activateEngineGraphTarget(target) {
  const keyMarker = target.closest?.("[data-engine-graph-key-position-ply]");
  if (keyMarker) {
    const ply = Number(keyMarker.dataset.engineGraphKeyPositionPly);
    const index = formView.findKeyPositionIndexByMoveNumber(ply);
    if (index >= 0) {
      stepNavigation.goTo(4);
      requestAnimationFrame(() => formView.focusKeyPosition(index, { field: "fact" }));
    }
    return true;
  }
...
document.getElementById("engine-evaluation-graph").addEventListener("click", (event) => {
  activateEngineGraphTarget(event.target);
});
document.getElementById("engine-evaluation-graph").addEventListener("keydown", (event) => {
  if (event.key !== "Enter" && event.key !== " ") return;
  if (activateEngineGraphTarget(event.target)) event.preventDefault();
});
