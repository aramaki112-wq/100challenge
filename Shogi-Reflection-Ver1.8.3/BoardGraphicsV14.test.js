...
test("駒SVGは全駒で共通の五角形外形を使う", () => {
  ...
  assert.match(svg, /<path class="piece-body" d="M50 5 C52 5 53\.5 5\.6 55\.1 6\.6/);
});
...
test("成駒は専用Classと赤文字Presentationで区別する", () => {
  const svg = shogiPieceSvg({ label: "馬", type: "BISHOP", promoted: true, rotated: false });
  assert.match(svg, /is-promoted/);
  assert.match(svg, />馬</);
  assert.match(fs.readFileSync(new URL("./style.css", import.meta.url), "utf8"), /\.replay-piece\.is-promoted \.piece-label[\s\S]*fill:#8b2f28/);
});
