...
export class BrowserEngineAnalysisView {
  constructor({ documentObject = document, graphView = new EngineEvaluationGraphView() } = {}) {
    ...
    this.graph = this.#required("engine-evaluation-graph");
  ...
  showUnanalyzed({ engineAvailable = null, message = "未解析" } = {}) {
    ...
    this.graph.innerHTML = '<p class="empty-state">解析後に、対局全体の評価推移をここへ表示します。</p>';
  ...
  showAnalyzing({ completed = 0, total = 1 } = {}) {
    ...
    this.graph.innerHTML = '<p class="empty-state">全plyの評価値を取得しています…</p>';
  ...
  setKeyPositions(keyPositions = []) {
    this.keyPositions = Array.isArray(keyPositions) ? keyPositions : [];
    if (this.lastResult) this.#renderGraph(this.lastResult);
  }

  #renderGraph(result) {
    this.graph.innerHTML = this.graphView.render({
  ...
  renderResult(result, { keyPositions = this.keyPositions } = {}) {
