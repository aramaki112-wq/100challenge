...
export class BrowserShogiReplayView {
  ...
  render(viewModel) {
    ...
    const squares = viewModel.squares.map((square, index) => {
      ...
      const piece = shogiPieceMarkup(square.piece, { containerClassName: "replay-piece-container" });
  ...
  scrollIntoView({ behavior = "smooth", reason = "GENERAL" } = {}) {
    const windowObject = this.document.defaultView ?? globalThis.window;
    const target = this.board.closest?.(".replay-board-shell") ?? this.board;
    const stickyNavigation = this.document.querySelector?.(".step-navigation");
    const stickyHeight = stickyNavigation?.getBoundingClientRect?.().height ?? 0;
    const topOffset = Math.max(20, stickyHeight + 16);

    if (
      typeof target?.getBoundingClientRect === "function" &&
      typeof windowObject?.scrollTo === "function"
    ) {
      const rect = target.getBoundingClientRect();
      const pageY = Number(windowObject.scrollY ?? windowObject.pageYOffset ?? 0);
      const top = Math.max(0, pageY + Number(rect.top ?? 0) - topOffset);
      windowObject.scrollTo({ top, behavior });
      return Object.freeze({ requested: true, reason, top, topOffset });
    }

    target?.scrollIntoView?.({ behavior, block: "start" });
    return Object.freeze({ requested: true, reason, fallback: true, topOffset });
  }
