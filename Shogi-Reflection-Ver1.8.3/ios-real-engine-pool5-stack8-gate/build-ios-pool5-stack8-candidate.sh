...
GATE_DIR="$APP_DIR/ios-real-engine-pool5-stack8-gate"
RUNTIME_DIR="$GATE_DIR/runtime-candidate"
OUT_DIR="$SOURCE_DIR/ios-pool5-stack8-build-output"
...
POOL_PATCH="$GATE_DIR/patches/yaneuraou-v9.00-pthread-pool-size-5.patch"
STACK_PATCH="$APP_DIR/ios-real-engine-stack8-gate/patches/yaneuraou-v9.00-stack-size-8m.patch"
...
EXPECTED_POOL_PATCH_SHA="8c1c829c9dcc7b3fdf82f93e8cdbefa3b93cbd4c0d987f43bf894589233acef1"
...
# A5-E6 is isolated from the checked-in / Formal engine area.
# Based on A5-E5, change only PTHREAD_POOL_SIZE from the measured A5-E5 runtime 4 -> 5.
# STACK_SIZE=8MiB and INITIAL_MEMORY=92,274,688 bytes remain fixed.
...
git apply --numstat "$POOL_PATCH" | tee "$EVIDENCE_DIR/pool5-patch-numstat.txt"
...
git -C "$SOURCE_DIR" diff --binary --abbrev=40 > "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL5-STACK8-CANDIDATE.patch"
sha256sum "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL5-STACK8-CANDIDATE.patch" > "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL5-STACK8-CANDIDATE.patch.sha256"
...
cp "$SOURCE_DIR/source/Makefile" "$CORR_DIR/YANEURAOU_SOURCE_MAKEFILE_POOL5_STACK8"
cp "$USI_PATCH" "$CORR_DIR/"
cp "$THREAD_PATCH" "$CORR_DIR/"
cp "$POOL_PATCH" "$CORR_DIR/"
cp "$STACK_PATCH" "$CORR_DIR/"

grep -q 'PTHREAD_POOL_SIZE=5' "$SOURCE_DIR/source/Makefile"
...
if grep -q '^.*LDFLAGS += -s PTHREAD_POOL_SIZE=1' "$SOURCE_DIR/source/Makefile"; then
  echo 'FAIL: active PTHREAD_POOL_SIZE=1 remains after A5-E6 candidate patch.' >&2
  exit 35
fi
if grep -q '^.*LDFLAGS += -s PTHREAD_POOL_SIZE=4' "$SOURCE_DIR/source/Makefile"; then
  echo 'FAIL: active PTHREAD_POOL_SIZE=4 remains after A5-E6 candidate patch.' >&2
  exit 37
fi
...
docker run --rm \
  ...
  bash -lc '
    set -euo pipefail
    test -z "${EMCC_CFLAGS:-}"
    emcc --version | tee ios-pool5-stack8-build-output/emcc-version.txt
    em++ --version | tee ios-pool5-stack8-build-output/empp-version.txt
    /emsdk/upstream/bin/clang --version | tee ios-pool5-stack8-build-output/clang-version.txt
    node --version | tee ios-pool5-stack8-build-output/node-version.txt
    python3 --version | tee ios-pool5-stack8-build-output/python-version.txt
    cd source
    make clean
    make -j2 tournament \
      COMPILER=em++ \
      ...
      TARGET=../ios-pool5-stack8-build-output/yaneuraou.material.js \
      EM_EXPORT_NAME=YaneuraOu_Material \
      MATERIAL_LEVEL=1 \
      EM_INITIAL_MEMORY_SIZE=92274688 \
      2>&1 | tee ../ios-pool5-stack8-build-output/make.log
...
# A5-E6 control: only PTHREAD_POOL_SIZE changes 4 -> 5; Stack8 and Initial Memory remain fixed.
grep -q 'PTHREAD_POOL_SIZE=5' "$EVIDENCE_DIR/make.log"
...
if grep -q 'PTHREAD_POOL_SIZE=1' "$EVIDENCE_DIR/make.log"; then
  echo 'FAIL: candidate linker command still contains pool size 1.' >&2
  exit 36
fi
if grep -q 'PTHREAD_POOL_SIZE=4' "$EVIDENCE_DIR/make.log"; then
  echo 'FAIL: candidate linker command still contains pool size 4.' >&2
  exit 38
fi
...
const metadata={
  schemaVersion:1,
  status:'IOS_POOL5_STACK8_CANDIDATE_NOT_FORMAL',
  purpose:'iPhone Safari Real YaneuraOu pthread bootstrap experiment after A5-E5 resolved factory/bridge/usiok, then created a fifth pthread Worker immediately after isready and timed out before readyok',
  ...
  experiment:{
    run:'A5-E6',
    changed:'PTHREAD_POOL_SIZE',
    baseline:4,
    candidate:5,
    stackSizeHeldAt:8388608,
  ...
  sourcePatches:[
    ...
    {file:'ios-real-engine-pool5-stack8-gate/patches/yaneuraou-v9.00-pthread-pool-size-5.patch',sha256:poolSha},
  ...
  checkoutBaseline:{
    ...
    note:'Candidate output is isolated under ios-real-engine-pool5-stack8-gate/runtime-candidate and never overwrites engine/yaneuraou.'
  },
  publicDistributionReady:false,
  commercialDistributionReady:false,
  legalReviewRequiredBeforePublicDistribution:true,
  note:'Run A5-E6 is a personal-device technical experiment only. Device success does not grant Formal or distribution status.'
};
if(metadata.pthreadPoolSize!==5) throw new Error(`expected pool5, got ${metadata.pthreadPoolSize}`);
...
fs.writeFileSync(path.join(gate,'IOS_POOL5_STACK8_CANDIDATE_METADATA.json'),JSON.stringify(metadata,null,2)+'\n');
NODE

sha256sum "$RUNTIME_DIR/"* > "$GATE_DIR/IOS_POOL5_STACK8_CANDIDATE_SHA256SUMS.txt"
...
echo 'PASS: A5-E6 candidate built with Pool5 + STACK_SIZE=8MiB; checked-in app Engine state remained untouched.'
