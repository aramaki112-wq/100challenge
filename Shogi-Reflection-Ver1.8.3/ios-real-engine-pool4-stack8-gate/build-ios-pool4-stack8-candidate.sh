...
GATE_DIR="$APP_DIR/ios-real-engine-pool4-stack8-gate"
RUNTIME_DIR="$GATE_DIR/runtime-candidate"
OUT_DIR="$SOURCE_DIR/ios-pool4-stack8-build-output"
...
POOL_PATCH="$GATE_DIR/patches/yaneuraou-v9.00-pthread-pool-size-4.patch"
...
EXPECTED_POOL_PATCH_SHA="f8b82eef754d44488b093c9b688cb0ec43b40985162204c6682eebb963b9a013"
...
# A5-E5 is isolated from the checked-in / Formal engine area.
# Based on A5-E4, change only PTHREAD_POOL_SIZE from the A5-E2 runtime baseline 1 -> 4.
...
git apply --numstat "$POOL_PATCH" | tee "$EVIDENCE_DIR/pool4-patch-numstat.txt"
...
git -C "$SOURCE_DIR" diff --binary --abbrev=40 > "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL4-STACK8-CANDIDATE.patch"
sha256sum "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL4-STACK8-CANDIDATE.patch" > "$CORR_DIR/YaneuraOu-ShogiReflection-IOS-POOL4-STACK8-CANDIDATE.patch.sha256"
...
cp "$SOURCE_DIR/source/Makefile" "$CORR_DIR/YANEURAOU_SOURCE_MAKEFILE_POOL4_STACK8"
cp "$USI_PATCH" "$CORR_DIR/"
cp "$THREAD_PATCH" "$CORR_DIR/"
cp "$POOL_PATCH" "$CORR_DIR/"
cp "$STACK_PATCH" "$CORR_DIR/"

grep -q 'PTHREAD_POOL_SIZE=4' "$SOURCE_DIR/source/Makefile"
...
if grep -q '^.*LDFLAGS += -s PTHREAD_POOL_SIZE=1' "$SOURCE_DIR/source/Makefile"; then
  echo 'FAIL: active PTHREAD_POOL_SIZE=1 remains after A5-E5 candidate patch.' >&2
  exit 35
fi
...
docker run --rm \
  ...
  bash -lc '
    set -euo pipefail
    test -z "${EMCC_CFLAGS:-}"
    emcc --version | tee ios-pool4-stack8-build-output/emcc-version.txt
    em++ --version | tee ios-pool4-stack8-build-output/empp-version.txt
    /emsdk/upstream/bin/clang --version | tee ios-pool4-stack8-build-output/clang-version.txt
    node --version | tee ios-pool4-stack8-build-output/node-version.txt
    python3 --version | tee ios-pool4-stack8-build-output/python-version.txt
    cd source
    make clean
    make -j2 tournament \
      COMPILER=em++ \
      ...
      TARGET=../ios-pool4-stack8-build-output/yaneuraou.material.js \
      EM_EXPORT_NAME=YaneuraOu_Material \
      MATERIAL_LEVEL=1 \
      EM_INITIAL_MEMORY_SIZE=92274688 \
      2>&1 | tee ../ios-pool4-stack8-build-output/make.log
...
# A5-E5 control: only PTHREAD_POOL_SIZE changes 1 -> 4; Stack8 and Initial Memory remain fixed.
grep -q 'PTHREAD_POOL_SIZE=4' "$EVIDENCE_DIR/make.log"
...
const metadata={
  schemaVersion:1,
  status:'IOS_POOL4_STACK8_CANDIDATE_NOT_FORMAL',
  purpose:'iPhone Safari Real YaneuraOu pthread bootstrap experiment after A5-E4 observed one loaded worker followed by three additional worker creations and factory stall',
  ...
  sourcePatches:[
    ...
    {file:'ios-real-engine-pool4-stack8-gate/patches/yaneuraou-v9.00-pthread-pool-size-4.patch',sha256:poolSha},
  ...
  checkoutBaseline:{
    ...
    note:'Candidate output is isolated under ios-real-engine-pool4-stack8-gate/runtime-candidate and never overwrites engine/yaneuraou.'
  },
  publicDistributionReady:false,
  commercialDistributionReady:false,
  legalReviewRequiredBeforePublicDistribution:true,
  note:'Run A5-E5 is a personal-device technical experiment only. Device success does not grant Formal or distribution status.'
};
if(metadata.pthreadPoolSize!==4) throw new Error(`expected pool4, got ${metadata.pthreadPoolSize}`);
...
fs.writeFileSync(path.join(gate,'IOS_POOL4_STACK8_CANDIDATE_METADATA.json'),JSON.stringify(metadata,null,2)+'\n');
NODE

sha256sum "$RUNTIME_DIR/"* > "$GATE_DIR/IOS_POOL4_STACK8_CANDIDATE_SHA256SUMS.txt"
...
echo 'PASS: A5-E5 candidate built with Pool4 + STACK_SIZE=8MiB; checked-in app Engine state remained untouched.'
