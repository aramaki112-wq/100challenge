...
for(const [file,hash] of [[m.jsFile,m.jsSha256],[m.wasmFile,m.wasmSha256],[m.workerFile,m.workerSha256]]){
  if(!file||!hash) throw new Error('asset metadata missing');
  const p=path.join('engine','yaneuraou',file); if(!fs.existsSync(p)) throw new Error(`missing ${p}`);
...
console.log('PASS: build metadata and asset hashes match.');
NODE
