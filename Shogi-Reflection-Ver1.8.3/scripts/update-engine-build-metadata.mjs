...
const oneWorker = () => fs.readdirSync(engineDir).filter(x => /^yaneuraou.*\.worker\.js$/.test(x)).sort();
...
let metadata = {
  ...
  jsFile: null, wasmFile: null, workerFile: null,
  jsSha256: null, wasmSha256: null, workerSha256: null,
  measured: built,
  notes: built ? [
...
if (built) {
  const js=path.join(engineDir,"yaneuraou.js"), wasm=path.join(engineDir,"yaneuraou.wasm"), workers=oneWorker();
  if (!fs.existsSync(js) || !fs.existsSync(wasm) || workers.length!==1) throw new Error("built metadata requires JS, WASM, and exactly one generated worker");
  if (readMaybe("yaneuraou-source-commit.txt") !== constants.commit) throw new Error("recorded source commit mismatch");
  metadata.jsFile=path.basename(js); metadata.wasmFile=path.basename(wasm); metadata.workerFile=workers[0];
  metadata.jsSha256=sha(js); metadata.wasmSha256=sha(wasm); metadata.workerSha256=sha(path.join(engineDir,workers[0]));
}
...
Object.assign(manifest, {
  ...
  pthreadWorkerUrl: built ? `./engine/yaneuraou/${metadata.workerFile}` : null,
  jsSha256: metadata.jsSha256, wasmSha256: metadata.wasmSha256, workerSha256: metadata.workerSha256,
  buildMetadataUrl: "./ENGINE_BUILD_METADATA.json",
  note: built ? "Official-source build/hash evidence is present. This enables runtime loading only; Formal Completion and public distribution remain separate gates." : "Distribution-safe manifest. available=true only after the official-source Build Bridge generates and hashes real assets."
});
fs.writeFileSync(manifestPath, JSON.stringify(manifest,null,2)+"\n");
console.log(JSON.stringify({status:metadata.status, js:metadata.jsFile, wasm:metadata.wasmFile, worker:metadata.workerFile},null,2));
