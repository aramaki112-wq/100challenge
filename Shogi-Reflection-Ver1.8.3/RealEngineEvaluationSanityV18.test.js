...
test("ReflectionLocal fallback Evaluation Sanity: 初期局面は有限CPとして返る", async () => {
...
test("ReflectionLocal fallback Evaluation Sanity: 明確な駒得/駒損でside-to-move視点の符号が反転する", async () => {
  const transport = await createTransport();
  try {
...
test("ReflectionLocal fallback Evaluation Sanity: 詰み局面はCPへ潰さずmateとして返る", async () => {
  const transport = await createTransport();
  try {
