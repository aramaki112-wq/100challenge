...
test("Import反映後に未保存であることを表示する", () => {
  assert.ok(main.includes("まだ保存済み対局やブラウザ保存データは変更されていません"));
});
...
test("KIF入力Clearと読み込みやり直しの操作を分離する", () => {
  assert.ok(html.includes('id="clear-kif-paste"'));
  assert.match(html, />入力をクリア<\/button>/);
  assert.match(html, />棋譜入力へ戻る<\/button>/);
  assert.match(main, /KifImportDraftResetController/);
  assert.match(main, /clearInput\(\)/);
  assert.match(main, /retryInput\(\)/);
});

test("KIF操作Buttonはスマートフォンで48px Touch Targetを持つ", () => {
  assert.match(style, /\.kif-paste-actions button,[\s\S]*min-height:48px/);
});
