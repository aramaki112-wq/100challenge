...
function keyPositionMarkup(index, keyPositionId = `KP-${index + 1}`) {
  ...
  return `
    <article class="key-position-card" data-key-position data-key-position-index="${index}">
      ...
      <label>盤面・局面メモ <small>局面記録とは別の本人入力</small><textarea data-field="boardState" rows="2" placeholder="駒の配置や直前の流れを自分の言葉で記録"></textarea></label>
      <div class="observation-grid">
        <label class="fact-field"><span>事実（FACT）</span><small>自動生成されません</small><textarea data-field="fact" rows="3" placeholder="盤上で実際に起きたこと"></textarea></label>
        <label class="interpretation-field"><span>解釈（INTERPRETATION）</span><small>自動生成されません</small><textarea data-field="interpretation" rows="3" placeholder="自分がどう受け取ったか"></textarea></label>
        <label class="hypothesis-field"><span>仮説（HYPOTHESIS）</span><small>自動生成されません</small><textarea data-field="hypothesis" rows="3" placeholder="別の見方・次に確かめる可能性"></textarea></label>
      </div>
      <div class="field-grid two-columns">
        ...
        <label>局面ごとの判断パターン <textarea data-field="decisionPattern" rows="3"></textarea></label>
...
export class BrowserGameReviewFormView {
  ...
  focusKeyPosition(index) {
    ...
    card.querySelector('[data-field="title"]')?.focus({ preventScroll: true });
  }
  ...
  renderSnapshotPreview(card) {
    ...
    try {
      ...
      const squares = model.squares.map((square) => {
        ...
        const piece = shogiPieceSvg(square.piece, { className: "snapshot-piece" });
