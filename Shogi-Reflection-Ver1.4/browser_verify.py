...
def run() -> int:
    ...
    with sync_playwright() as p:
        ...
        checks.check("Ver1.4 title", "Ver.1.4" in page.title(), page.title())
        ...
        # Register KIF -> Step2 and save game only.
        page.fill("#kif-paste-text", basic_kif)
        page.click("#preview-kif-paste")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        ...
        checks.check("Game-only status visible", "棋譜のみ" in page.locator("#saved-review-list").inner_text())
        ...
        # Add 3 KeyPositions and verify Step data survives navigation.
        for move in (1, 3, 5):
            jump(page, move)
    ...
    result_lines = [
        "Shogi Reflection Ver.1.4 Browser Verification",
        "==============================================",
