    ...
    this.list.innerHTML = viewModel.items.map((item) => `
      <article class="review-list-card" data-review-card="${escapeHtml(item.reviewId)}">
        <button type="button" class="review-card-main" data-view-review="${escapeHtml(item.reviewId)}">
          ...
          <strong>${escapeHtml(item.opponentLabel)}｜${escapeHtml(item.sideLabel)}・${escapeHtml(item.resultLabel)}</strong>
          <span>${item.moveCount}手｜${escapeHtml(item.timeControlLabel)}</span>
  ...
  renderDetail(detail) {
    ...
    this.detail.innerHTML = `
      <div class="detail-heading-row">
        <div><p class="eyebrow">保存済み対局</p><h3>${escapeHtml(detail.displayDate)}の対局</h3><p>${escapeHtml(detail.opponentLabel)}｜${escapeHtml(detail.sideLabel)}・${escapeHtml(detail.resultLabel)}｜${detail.moveCount}手</p><span class="workflow-badge" data-status="${escapeHtml(detail.workflowStatus)}">${escapeHtml(detail.workflowStatusLabel)}</span></div>
