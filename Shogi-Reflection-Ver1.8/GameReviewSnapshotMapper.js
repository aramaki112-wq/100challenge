...
export function gameReviewFromPersistentSnapshot(reviewSnapshot) {
  ...
  try {
    return new GameReview({
      ...
      senteName: reviewSnapshot.senteName ?? "",
      goteName: reviewSnapshot.goteName ?? "",
      ...
      workflowStatus: reviewSnapshot.workflowStatus,
      createdAt: reviewSnapshot.createdAt ?? null,
      updatedAt: reviewSnapshot.updatedAt ?? null
    });
  } catch (error) {
    if (error instanceof ReflectionError || error instanceof WorkflowError || error instanceof KeyPositionReplayError || error instanceof TypeError) {
      throw new PersistenceError(
        PERSISTENCE_ERROR_CODES.DOMAIN_RULE_VIOLATION,
