...
def run() -> int:
    ...
    with sync_playwright() as p:
        ...
        checks.check("Ver1.6 title", "Ver.1.6" in page.title(), page.title())
        ...
        # Register KIF -> Step2 and save game only. Use opening header for Ver.1.6 summary verification.
        ...
        checks.check("Rounded piece outline", piece_path.startswith('M50 5 Q52 5 54 6'), piece_path)
        ...
        goto_step(page, 4)
        checks.check("Engine Analyze button touch target", page.locator("#analyze-game").bounding_box()["height"] >= 44)
        ...
        checks.check("Engine candidate displayed", page.locator("[data-engine-candidate]").count() >= 1)
        ...
        checks.check("Candidate to Replay", "STEP 3 / 7" in page.locator("#step-current-status").inner_text())
        goto_step(page, 4)
        ...
        # Cancel can interrupt a re-analysis.
    ...
    result_lines = [
        "Shogi Reflection Ver.1.6 Browser Verification",
        "==============================================",
