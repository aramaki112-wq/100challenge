...
document.getElementById("replay-move-list").addEventListener("click", (event) => {
  ...
  // Explicit user selection is the only Move List action allowed to return the page to the board.
...
// KeyPosition
function addCurrentReplayPositionToKeyPosition() {
  ...
  const input = formView.readInput();
  const result = keyPositionReplayController.add({ sourceGameId: input.reviewId, sourceKifText: input.kifuText });
  replayView.setKeyPositionCount(formView.getMeaningfulKeyPositionCount());
  if (result.status === "REJECTED") {
    replayView.showAddResult({ kind: "error", message: `${result.viewError.message}（${result.viewError.code}）` });
    if (result.duplicateIndex !== undefined) formView.focusKeyPosition(result.duplicateIndex);
    return result;
  }
  currentHasUnsavedChanges = true;
  ...
  formView.showFeedback({ kind: result.hasWarning ? "warning" : "success", title: "現在局面を重要局面候補へ追加しました", message: "局面記録だけを反映しました。FACT／INTERPRETATION／HYPOTHESISは自分で入力してください。", focus: false });
  return result;
}
...
document.getElementById("analyze-game").addEventListener("click", async () => {
  ...
  try {
    ...
    engineAnalysisView.showAnalyzing({ completed: 0, total: history.maxMoveNumber + 1 });
...
document.getElementById("engine-analysis-candidates").addEventListener("click", (event) => {
  ...
  if (replayButton) {
    ...
    if (rendered) { navigateToStep(3); replayView.scrollIntoView({ behavior: "smooth" }); }
    return;
  }
  ...
  if (addButton) {
    try {
      ...
      if (rendered) addCurrentReplayPositionToKeyPosition();
