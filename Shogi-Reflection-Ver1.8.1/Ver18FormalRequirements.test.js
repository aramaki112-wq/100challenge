...
test("verified manifest時はYaneuraOuWasm primary + Local fallbackを構成する", async () => {
  ...
  const engine = await resolveBrowserEngine({
    location: { href: "https://example.test/app/" },
    Worker: DummyWorker,
    async fetch() { return { ok: true, async json() { return { available: true, workerUrl: "./YaneuraOuWasmWorkerBootstrap.js", engineVersion: "V9.00", evaluationModel: "MATERIAL", materialLevel: 1 }; } }; }
  });
