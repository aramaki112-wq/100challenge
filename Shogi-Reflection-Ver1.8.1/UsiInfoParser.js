...
export function parseUsiInfoLine(line) {
  ...
  const infoKeywords = new Set([
    ...
    "currmovenumber", "hashfull", "nps", "tbhits", "sbhits", "cpuload", "string", "refutation",
    "currline"
  ]);
  const pv = [];
  if (pvIndex >= 0) {
    for (let i = pvIndex + 1; i < tokens.length; i += 1) {
      if (infoKeywords.has(tokens[i])) break;
      pv.push(tokens[i]);
    }
  }
