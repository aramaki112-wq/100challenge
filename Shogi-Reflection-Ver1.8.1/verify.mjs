...
const baseline = JSON.parse(fs.readFileSync(path.join(root, "SOURCE_OF_TRUTH_V1_8_BASELINE_HASHES.json"), "utf8"));
...
check("User-supplied Ver.1.8 Baseline file count", baseline.fileCount === 331 && baseFiles.length === 331, `${baseline.fileCount}/${baseFiles.length}`);
...
const required = [
  ...
  "scripts/build-yaneuraou-wasm.sh", "scripts/finalize-yaneuraou-manifest.mjs",
...
const report = [
  ...
  "Date: 2026-08-09",
  "Source of Truth: user-supplied Shogi-Reflection-Ver1.8(1).zip",
  "",
  `Files: ${files.length}`,
