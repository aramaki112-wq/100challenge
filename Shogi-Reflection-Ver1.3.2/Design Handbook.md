...
# Ver.1.3.2 追補 — Replay UX・Smartphone操作改善

## 第1部　問題を観測する

### STEP 1　Page ScrollとMove List Scrollを区別する

#### 1. 🎯 このSTEPの目的
Replay中に「何が動くべきか」を明確にする。

#### 2. 🤔 なぜこの作業をするのか
現在手Highlightは必要だが、そのためにBrowser Page全体が動く必要はない。盤面を見続ける利用者のViewportと、棋譜一覧内部の追従は別Stateだからである。

#### 3. 💻 コードを書く
`BrowserShogiReplayView`のCurrent Move追従箇所を監査し、`scrollIntoView()`を自動追従から除去する。

#### 4. 💡 設計者のひとこと
「見せたいElementがある」と「Pageをそこへ移動する」は同じ要求ではない。

#### 5. ✅ チェックポイント
- Current Move Highlightは残っているか。
- `window.scrollY`をNavigationが変更していないか。
- Move List内部は必要に応じて追従できるか。

#### 6. ▶ 次へ進む条件
Page ScrollとContainer Scrollを別責務として説明できること。

## 第2部　責務を分離する

### STEP 2　ReplayScrollPolicyを作る

#### 1. 🎯 このSTEPの目的
現在手の可視性判定とContainer内部ScrollをViewから分離する。

#### 2. 🤔 なぜこの作業をするのか
場当たり的な`window.scrollTo()`で位置を戻すと、原因となる副作用を残したまま後処理だけが増える。最初からScroll対象をMove List Containerへ限定する方が責務が明確である。

#### 3. 💻 コードを書く
`ReplayScrollPolicy`でContainer RectとCurrent Item Rectを比較し、見切れた時だけ`container.scrollTop`を更新する。戻り値には`pageScrollRequested: false`を含め、Policy境界を明示する。

#### 4. 💡 設計者のひとこと
UI PolicyもDomainと同じく「何をしないか」を契約に含めると安全になる。

#### 5. ✅ チェックポイント
- Itemが見えている時はScrollしないか。
- 上下の見切れを処理できるか。
- Container / Item不在で例外的なData変更をしないか。

#### 6. ▶ 次へ進む条件
Policy Unit TestでPage Scroll要求が0であることを確認できること。

## 第3部　SmartphoneとFocusを整える

### STEP 3　盤面とNavigationを同じ操作面に置く

#### 1. 🎯 このSTEPの目的
390px前後のPortraitで片手Replayを続けやすくする。

#### 2. 🤔 なぜこの作業をするのか
Buttonが盤面から離れたりTouch Targetが小さいと、Scroll問題を直しても連続Replayの負荷は残るためである。

#### 3. 💻 コードを書く
390px前後ではNavigationを4列、各Buttonを48px以上とする。持ち駒欄・盤面Paddingを小さくし、明示的にReplayへ移動する時だけ盤面を中央寄せする。

#### 4. 💡 設計者のひとこと
Smartphoneでは「一度の操作ができる」より「同じ操作を50回続けられる」ことが重要になる。

#### 5. ✅ チェックポイント
- 盤面とNavigationが同一Viewportへ収まる位置を作れるか。
- 48px Touch Targetを確保しているか。
- 盤面反転後も配置が崩れないか。

#### 6. ▶ 次へ進む条件
390px Browser Testで50手連続Navigationが成立すること。

### STEP 4　FocusをScrollから切り離す

#### 1. 🎯 このSTEPの目的
Feedback表示やFocus変更がReplayのViewportを奪わないようにする。

#### 2. 🤔 なぜこの作業をするのか
FocusはAccessibilityに必要だが、BrowserはFocus対象を見せるためにPageをScrollすることがある。重要局面追加成功のたびにForm FeedbackへFocusするとReplay Loopが途切れる。

#### 3. 💻 コードを書く
`showFeedback`へFocus可否を渡せるようにし、Replayから重要局面を追加した成功FeedbackではFocusしない。重複カードへ意図的に移動した後のInput Focusには`preventScroll: true`を使う。

#### 4. 💡 設計者のひとこと
Accessibilityは「常にFocusを移すこと」ではなく、利用者の現在作業を壊さず状態変化を伝えることまで含む。

#### 5. ✅ チェックポイント
- 重要局面追加成功後にPage位置が残るか。
- 重複時には既存カードへ移動できるか。
- Focusの二重Scrollがないか。

#### 6. ▶ 次へ進む条件
重要局面追加→次へ、のLoopをBrowserで確認できること。

## 第4部　回帰を証明する

### STEP 5　Replay UXをBrowser数値で検証する

#### 1. 🎯 このSTEPの目的
「飛ばなくなった」を目視だけでなく測定可能にする。

#### 2. 🤔 なぜこの作業をするのか
Scroll問題はViewport、棋譜長、Focus、操作経路で再発しやすい。`window.scrollY`とMove List `scrollTop`を別々に測ることで責務分離を検証できる。

#### 3. 💻 コードを書く
300手Fixtureを用意し、390px Chromiumで次へ50回、前へ10回、最初／最後、Keyboard、100/200/300手Jump、反転、重要局面追加を実行する。通常Navigation前後で`window.scrollY`が維持され、Move List `scrollTop`が必要に応じて変化することをAssertする。

#### 4. 💡 設計者のひとこと
UIの「自然さ」も、重要な部分は数値へ落とせる。

#### 5. ✅ チェックポイント
- 既存Testをすべて継承したか。
- KIF Paste / File / Drag & Dropを維持したか。
- Backup/Restore、Markdown、Observation Cardを維持したか。
- Missing Importが0か。

#### 6. ▶ 次へ進む条件
ZIPを別Folderへ展開し、展開物だけで全検証が成功すること。
