...
Ver.1.3.1では現在手を棋譜一覧で強調した直後に`scrollIntoView()`を使用していたため、Smartphoneの縦配置では棋譜一覧だけでなくBrowser Pageまで下へ動くことがあった。

Ver.1.3.2では`ReplayScrollPolicy`を追加し、現在手追従を棋譜一覧Containerの`scrollTop`更新へ限定した。Replay State更新、Current Move Highlight、Move List Scroll、Page Scrollを別責務として扱う。

## 重要な変更

- 通常NavigationはPage Scrollを要求しない。
- Current Move IDを`replay-move-{手数}`としてView Modelから生成する。
- 同じ棋譜をNavigation中はMove List全体を毎回再構築せず、Current class/ARIAだけを更新する。
- 棋譜一覧のCurrent Moveが見切れた場合だけContainer内部を追従する。
- 棋譜一覧を利用者自身がタップした場合だけ、選択後に盤面へ戻す。
- 重要局面追加成功時はForm FeedbackへFocusを移さずReplay位置を維持する。
- Smartphone Navigationは48px Touch Targetを確保する。

Domain Model、Repository、Snapshot、Backup/Restore、Markdown Export、Observation CardのSchemaは変更していない。
