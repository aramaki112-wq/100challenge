...
# Ver.1.3.2 Challenge — Replay Scroll Responsibility

## Challenge

現在手を棋譜一覧で追従させながら、盤面を見ているBrowser PageのViewportは動かさない実装を作る。

## 制約

- GameReview Domainを変更しない。
- Replay StateとScroll Stateを混在させない。
- `window.scrollTo()`で副作用を後から戻す実装にしない。
- KIF Paste、File Import、Drag & Drop、重要局面、Backup、Markdownを壊さない。

## 完了基準

300手棋譜・390px Viewportで50手連続Navigationを行い、Page `scrollY`が維持され、Move List内部だけがCurrent Moveへ追従すること。
