...
## Ver.1.3.2 Replay UX・Smartphone操作改善

Ver.1.3.2はVer.1.3.1をSource of Truthとして、棋譜再現盤のPage Scroll副作用を修正した小規模UX Releaseです。

### 主な変更

- Current Move追従の`scrollIntoView()`を廃止
- `ReplayScrollPolicy`追加
- Page ScrollとMove List内部Scrollを分離
- Current Move ID / `aria-current` / `aria-selected`追加
- 同一棋譜Navigation中のMove List全DOM再構築を抑制
- 390px前後でNavigation Button 48px Touch Target
- Replayへ明示移動する時は盤面を中央寄せ
- 重要局面追加成功時にForm FeedbackがReplay Focusを奪わない
- 300手Fixtureによる長棋譜Browser Test追加
...
詳細は`REPLAY_SCROLL_POLICY.md`、`MOBILE_REPLAY_UX.md`、`Ver.1.3.2操作手順書.md`を参照してください。
