...
## Ver.1.3.2 Replay UX Review Checklist

- [ ] `scrollIntoView()`をCurrent Move自動追従へ使用していない。
- [ ] 次へ／前へ／最初へ／最後へでPage `scrollY`が棋譜一覧へ飛ばない。
- [ ] Keyboard NavigationでもPage Scrollしない。
- [ ] 任意JumpでもPage位置を不必要に失わない。
- [ ] Current Move Highlightが残っている。
- [ ] Current Moveが見切れた場合だけMove List `scrollTop`が変化する。
- [ ] Move List Tap後は盤面へ戻れる。
- [ ] 390px前後でNavigation Buttonが48px以上ある。
- [ ] 盤面反転してもScroll Policyが同じである。
- [ ] 重要局面追加成功後もReplayを続けられる。
- [ ] KIF File / Drag & Drop / Paste / Clipboardが維持されている。
- [ ] Replay NavigationだけではGameReviewを保存しない。
- [ ] Backup/Restore、Markdown、Observation Card互換を維持する。
- [ ] 100/200/300手の長い棋譜で内部Scrollを確認する。
