...
def run() -> int:
    ...
    with sync_playwright() as p:
        ...
        checks.check("Application title", page.title() == "将棋振り返りアプリ Ver.1.3.3｜KIF入力やり直し・日本語UI改善", page.title())
        ...
        checks.check("KIF paste textarea", page.locator("#kif-paste-text").count() == 1)
        checks.check("Clipboard read button", page.locator("#read-kif-clipboard").count() == 1)
        checks.check("Pasted KIF preview button", page.locator("#preview-kif-paste").count() == 1)
        checks.check("KIF clear button", page.locator("#clear-kif-paste").count() == 1)
        checks.check("KIF retry button Japanese label", page.locator("#cancel-kif-import").inner_text() == "棋譜入力へ戻る")
        checks.check("KIF clear button Japanese label", page.locator("#clear-kif-paste").inner_text() == "入力をクリア")
        checks.check("Japanese replay navigation aria-label", page.locator(".replay-navigation").get_attribute("aria-label") == "棋譜操作")
        ...
        paste_text = (ROOT / "fixtures/replay-basic.kif").read_text(encoding="utf-8")
        page.fill("#kif-paste-text", paste_text)
        page.click("#preview-kif-paste")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        pasted_preview = page.locator("#kif-import-preview").inner_text()
        checks.check("Manual paste preview", "clipboard-paste.kifu" in pasted_preview)
        checks.check("Manual paste parses players", "先手太郎" in pasted_preview and "後手花子" in pasted_preview)
        checks.check("Manual paste remains unsaved", page.locator("#saved-count").inner_text() == "0")
        page.click("#cancel-kif-import")
        checks.check("Retry returns to KIF input", page.locator("#kif-import-preview").is_hidden())
        checks.check("Retry preserves pasted text", page.locator("#kif-paste-text").input_value() == paste_text)
        alternate_text = (ROOT / "fixtures/normal-resign-utf8.kifu").read_text(encoding="utf-8")
        page.fill("#kif-paste-text", alternate_text)
        page.click("#preview-kif-paste")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        retry_preview = page.locator("#kif-import-preview").inner_text()
        checks.check("Retry accepts different KIF", "勇太" in retry_preview and "ぴよ帝" in retry_preview)
        checks.check("Retry removes stale Preview", "先手太郎" not in retry_preview and "後手花子" not in retry_preview)
        clipboard_before_clear = page.evaluate("navigator.clipboard.readText()")
        storage_before_clear = page.evaluate(f"localStorage.getItem('{STORAGE_KEY}')")
        page.click("#clear-kif-paste")
        checks.check("KIF clear empties pasted text", page.locator("#kif-paste-text").input_value() == "")
        checks.check("KIF clear hides Preview", page.locator("#kif-import-preview").is_hidden())
        checks.check("KIF clear does not create saved review", page.locator("#saved-count").inner_text() == "0")
        checks.check("KIF clear does not change LocalStorage", page.evaluate(f"localStorage.getItem('{STORAGE_KEY}')") == storage_before_clear)
        checks.check("KIF clear does not change Clipboard", page.evaluate("navigator.clipboard.readText()") == clipboard_before_clear)

        page.evaluate("text => navigator.clipboard.writeText(text)", paste_text)
        page.click("#read-kif-clipboard")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        checks.check("Clipboard button fills textarea", "先手：先手太郎" in page.locator("#kif-paste-text").input_value())
        checks.check("Clipboard button previews KIF", "clipboard-paste.kifu" in page.locator("#kif-import-preview").inner_text())
        checks.check("Clipboard import remains unsaved", page.locator("#saved-count").inner_text() == "0")
        page.click("#cancel-kif-import")
        ...
        checks.check("Pasted KIF applied to Form", "先手：先手太郎" in page.locator('[name="kifuText"]').input_value())
        checks.check("Pasted KIF reaches replay board", page.locator(".replay-square").count() == 81)
        ...
        checks.check("Replay origin visible", "棋譜再現から追加" in first_card.locator("[data-replay-origin]").inner_text())
        ...
        saved_storage_before_clear = page.evaluate(f"localStorage.getItem('{STORAGE_KEY}')")
        page.evaluate("text => navigator.clipboard.writeText(text)", "SAVED-CLIPBOARD-KIF")
        page.fill("#kif-paste-text", paste_text)
        page.click("#preview-kif-paste")
        page.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        page.click("#clear-kif-paste")
        checks.check("Clear preserves saved GameReview count", page.locator("#saved-count").inner_text() == "1")
        checks.check("Clear preserves saved LocalStorage snapshot", page.evaluate(f"localStorage.getItem('{STORAGE_KEY}')") == saved_storage_before_clear)
        checks.check("Clear preserves Clipboard after saved review", page.evaluate("navigator.clipboard.readText()") == "SAVED-CLIPBOARD-KIF")
        ...
        checks.check("Warning origin visible", "注意あり" in page_warning.locator("[data-replay-origin]").first.inner_text())
        checks.check("Failure move not selectable", page_warning.locator("#replay-jump-number").get_attribute("max") == "2")

        # Ver.1.3.2 long Replay Scroll policy: Page Scroll must remain stable while only the Move List follows.
        page_scroll = browser.new_page(viewport={"width": 390, "height": 844})
        page_scroll.on("console", lambda msg: browser_logs.append(f"scroll console {msg.type}: {msg.text}"))
        page_scroll.on("pageerror", lambda err: browser_logs.append(f"scroll pageerror: {err}"))
        page_scroll.set_content(build_html(), wait_until="load")
        wait_initialized(page_scroll)
        long_kif = (ROOT / "fixtures/replay-long-300.kif").read_text(encoding="utf-8")
        page_scroll.fill("#kif-paste-text", long_kif)
        page_scroll.click("#preview-kif-paste")
        page_scroll.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        page_scroll.click("#apply-kif-import")
        page_scroll.wait_for_function("document.querySelector('#replay-jump-number')?.max === '300'")
        page_scroll.wait_for_timeout(1200)
        baseline_y = page_scroll.evaluate("window.scrollY")
        baseline_board_top = page_scroll.locator("#shogi-board").bounding_box()["y"]
        nav_box = page_scroll.locator(".replay-navigation").bounding_box()
        board_box = page_scroll.locator("#shogi-board").bounding_box()
        checks.check("390px board and navigation share viewport", board_box["y"] < 844 and nav_box["y"] < 844, str({"board": board_box, "nav": nav_box}))
        touch_height = page_scroll.locator("#replay-next").bounding_box()["height"]
        checks.check("Smartphone navigation touch target >= 48px", touch_height >= 48, str(touch_height))

        page_scroll.evaluate("""() => { for (let i = 0; i < 50; i += 1) document.querySelector('#replay-next').click(); }""")
        checks.check("50-step continuous next navigation", "50手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
        checks.check("Next navigation keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - baseline_y) <= 1, str({"before": baseline_y, "after": page_scroll.evaluate("window.scrollY")}))
        checks.check("Next navigation keeps board viewport position", abs(page_scroll.locator("#shogi-board").bounding_box()["y"] - baseline_board_top) <= 1)
        checks.check("Move List follows internally after 50 moves", page_scroll.evaluate("document.querySelector('#replay-move-list').scrollTop") > 0, str(page_scroll.evaluate("document.querySelector('#replay-move-list').scrollTop")))
        checks.check("Current move highlight after 50 moves", page_scroll.locator('#replay-move-50.is-current[aria-current="true"]').count() == 1)

        y_before_prev = page_scroll.evaluate("window.scrollY")
        page_scroll.evaluate("""() => { for (let i = 0; i < 10; i += 1) document.querySelector('#replay-previous').click(); }""")
        checks.check("10-step continuous previous navigation", "40手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
        checks.check("Previous navigation keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - y_before_prev) <= 1)

        for selector, label, expected in (("#replay-first", "First", 0), ("#replay-last", "Last", 300)):
            before = page_scroll.evaluate("window.scrollY")
            page_scroll.evaluate(f"document.querySelector('{selector}').click()")
            checks.check(f"{label} navigation status", f"{expected}手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
            checks.check(f"{label} navigation keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - before) <= 1)

        page_scroll.evaluate("document.body.focus()")
        before_keyboard = page_scroll.evaluate("window.scrollY")
        page_scroll.keyboard.press("ArrowLeft")
        checks.check("Keyboard navigation long replay", "299手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
        checks.check("Keyboard navigation keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - before_keyboard) <= 1)

        for move in (100, 200, 300):
            before = page_scroll.evaluate("window.scrollY")
            page_scroll.evaluate("""move => {
              const input = document.querySelector('#replay-jump-number');
              input.value = String(move);
              document.querySelector('#replay-jump-button').click();
            }""", move)
            checks.check(f"Long replay jump {move}", f"{move}手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
            checks.check(f"Long replay jump {move} keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - before) <= 1)

        page_scroll.evaluate("""() => {
          const input = document.querySelector('#replay-jump-number');
          input.value = '120';
          document.querySelector('#replay-jump-button').click();
        }""")
        before_flip = page_scroll.evaluate("window.scrollY")
        page_scroll.evaluate("document.querySelector('#replay-flip').click()")
        checks.check("Flip keeps Page scrollY", abs(page_scroll.evaluate("window.scrollY") - before_flip) <= 1)
        checks.check("Flip keeps current move", "120手目 / 300手" in page_scroll.locator("#replay-status").inner_text())

        before_add = page_scroll.evaluate("window.scrollY")
        page_scroll.evaluate("document.querySelector('#add-current-position').click()")
        checks.check("Important position add keeps Replay page position", abs(page_scroll.evaluate("window.scrollY") - before_add) <= 1)
        checks.check("Important position add remains unsaved in long replay", page_scroll.locator("#saved-count").inner_text() == "0")
        page_scroll.evaluate("document.querySelector('#replay-next').click()")
        checks.check("Navigation continues after important position add", "121手目 / 300手" in page_scroll.locator("#replay-status").inner_text())

        # Move-list tap is the intentional exception: after choosing a move, return to the Replay panel.
        page_scroll.evaluate("document.querySelector('#replay-move-200').scrollIntoView({behavior:'auto', block:'center'})")
        away_y = page_scroll.evaluate("window.scrollY")
        page_scroll.evaluate("document.querySelector('#replay-move-200').click()")
        page_scroll.wait_for_timeout(500)
        returned_y = page_scroll.evaluate("window.scrollY")
        checks.check("Move list tap jumps to selected move", "200手目 / 300手" in page_scroll.locator("#replay-status").inner_text())
        checks.check("Move list tap returns toward Replay panel", returned_y < away_y, str({"away": away_y, "returned": returned_y}))
        checks.check("Move list internal scroll remains independent", page_scroll.evaluate("document.querySelector('#replay-move-list').scrollTop") > 0)
        ...
        paste_box = page_mobile.locator("#kif-paste-text").bounding_box()
        checks.check("Smartphone KIF paste textarea visible", page_mobile.locator("#kif-paste-text").is_visible())
        checks.check("Smartphone KIF paste fits viewport", paste_box is not None and paste_box["width"] <= 390, str(paste_box))
        paste_font_size = page_mobile.evaluate("getComputedStyle(document.querySelector('#kif-paste-text')).fontSize")
        checks.check("Smartphone KIF paste avoids iOS zoom", paste_font_size == "16px", paste_font_size)
        checks.check("Smartphone clipboard button visible", page_mobile.locator("#read-kif-clipboard").is_visible())
        checks.check("Smartphone manual preview button visible", page_mobile.locator("#preview-kif-paste").is_visible())
        checks.check("Smartphone clear button visible", page_mobile.locator("#clear-kif-paste").is_visible())
        checks.check("Smartphone clear touch target >= 48px", page_mobile.locator("#clear-kif-paste").bounding_box()["height"] >= 48)
        page_mobile.fill("#kif-paste-text", (ROOT / "fixtures/replay-basic.kif").read_text(encoding="utf-8"))
        page_mobile.click("#preview-kif-paste")
        page_mobile.wait_for_function("!document.querySelector('#kif-import-preview').hidden")
        checks.check("Smartphone retry button visible", page_mobile.locator("#cancel-kif-import").is_visible())
        checks.check("Smartphone retry touch target >= 48px", page_mobile.locator("#cancel-kif-import").bounding_box()["height"] >= 48)
        page_mobile.click("#cancel-kif-import")
        jump(page_mobile, 3)
    ...
    report = [
        "Shogi Reflection Ver.1.3.3 Browser Verification",
        "Date: 2026-08-08",
        "",
        "Execution method:",
        "- Chromium headless via Playwright",
        "- Original index.html, style.css and ES Modules loaded through an Import Map",
        "- Native File, DataTransfer, DragEvent, Clipboard readText, textarea paste path, Keyboard, JSON Backup and responsive viewport boundaries",
        "- Snapshot add, duplicate prevention, five-item limit, save/reload, legacy data and atomic restore",
        "- Ver.1.3.3 KIF clear/retry data safety, Japanese UI, Ver.1.3.2 Page scrollY stability, 300-move Replay, Move List internal scroll and 390px touch target",
        "",
    ]
    report.extend(
