# 将棋振り返りアプリ Ver.1.3 — 棋譜再現盤・重要局面登録接続
...
Ver.1.3では、Ver.1.2のPosition Historyから現在局面と直前局面を取得し、客観的な局面Snapshotだけを既存KeyPositionへ任意参照として接続します。追加Buttonを押してもRepositoryへ保存せず、従来の保存Buttonを押したときだけGameReviewを正式更新します。
...
- 入力ZIP：`Shogi-Reflection-Ver1.2(2).zip`
- Ver.1.2実File：173件
- Ver.1.2既存Automated Test：333件成功／0件失敗
- Ver.1.3追加Automated Test：125件成功／0件失敗
- Ver.1.3全Automated Test：458件成功／0件失敗
- 実Chromium確認：116件成功／0件失敗
- Missing Import：0件
- Ver.1.2 File削除：0件

正式なFile分類とHash確認は`SOURCE_OF_TRUTH_AUDIT.md`を参照してください。
...
1. KIFをFile選択またはDrag & Dropする。
2. Import Previewを確認してFormへ反映する。
3. 棋譜再現盤を目的の手数へ移動する。
4. `この局面を重要局面へ追加`を押す。
...
追加しただけでは保存されません。詳しくは`Ver.1.3操作手順書.md`を参照してください。
