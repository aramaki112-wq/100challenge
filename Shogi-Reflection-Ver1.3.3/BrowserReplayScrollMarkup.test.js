...
test("Move List Jumpだけは利用者操作としてReplay盤面へ戻す", () => {
  assert.match(main, /replay-move-list[\s\S]*replayController\.jump[\s\S]*replayView\.scrollIntoView/);
});
...
test("390px相当ではNavigationを4列に保ち盤面直下で操作できる", () => {
  assert.match(css, /@media \(max-width:800px\)[\s\S]*replay-navigation[\s\S]*repeat\(4,minmax\(0,1fr\)\)/);
});
