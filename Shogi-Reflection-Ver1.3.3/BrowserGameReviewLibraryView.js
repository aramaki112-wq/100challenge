...
function exportButtons(reviewId, readyForNextGame) {
  ...
  const cardTitle = readyForNextGame
    ? "次局用Observation Cardを確認します。"
    : "重要局面3〜5件、次局の観察テーマ、次局で守るルールが揃うと作成できます。";
...
function missingGuidance(items = []) {
  ...
  if (!items.length) {
    return `<div class="reflection-guidance ready"><strong>次局用Observation Cardを作成できます</strong><p>次局で観察するテーマと守るルールをMarkdownへ変換できます。</p></div>`;
  }
  return `<div class="reflection-guidance"><strong>次局用Observation Card作成まで残っている項目</strong><ul>${items.map((item) => `<li>${escapeHtml(labels[item] ?? item)}</li>`).join("")}</ul></div>`;
}
...
export class BrowserGameReviewLibraryView {
    ...
    this.list.innerHTML = viewModel.items.map((item) => `
      <article class="review-list-card" data-review-card="${escapeHtml(item.reviewId)}">
        <button type="button" class="review-card-main" data-view-review="${escapeHtml(item.reviewId)}">
          ...
          <span class="review-card-meta">重要局面 ${item.keyPositionCount}件・次局ルール ${item.actionRuleCount}件</span>
  ...
  renderDetail(detail) {
    ...
    const actionRules = detail.actionRules.length
      ...
      : `<p class="empty-text">次局で守るルールはまだありません。</p>`;

    this.detail.innerHTML = `
      <div class="detail-heading-row">
        <div><p class="eyebrow">保存済み対局</p><h3>${escapeHtml(detail.displayDate)}の振り返り</h3><p>${escapeHtml(detail.sideLabel)}・${escapeHtml(detail.resultLabel)}｜${escapeHtml(detail.opponentLabel)}</p></div>
      ...
      <section class="detail-section"><h4>次局で守るルール</h4>${actionRules}</section>
  ...
  clearDetail() {
    this.detail.innerHTML = `<div class="library-empty"><strong>対局を選択してください</strong><p>一覧のカードを押すと、棋譜・重要局面・次局の観察テーマ・次局で守るルールとMarkdown出力を確認できます。</p></div>`;
  }
}
