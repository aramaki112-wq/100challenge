...
const kifImportDraftResetController = new KifImportDraftResetController({
  importController: kifImportController,
  view: kifImportView
});
...
function applyPendingKifImport() {
  try {
    ...
    kifImportView.clearPreview(
      "KIFを入力フォームへ反映しました。まだ保存済み対局やブラウザ保存データは変更されていません。振り返り内容を確認・追記してから保存してください。"
    );
    formView.showFeedback({
      kind: result.warnings.length > 0 ? "warning" : "success",
      title: "KIFを入力フォームへ反映しました",
      message: "KIF読み込み成功と保存成功は別です。対局の物語・重要局面・次局の観察テーマ・次局で守るルールを確認してから保存してください。",
...
function refreshLibrary({ selectedReviewId = null } = {}) {
  ...
  if (selectedReviewId && result.gameReviews.some((item) => item.reviewId === selectedReviewId)) {
    ...
    replayView.showUnavailable(
      "KIFを入力フォームへ反映するか、保存済み対局を選択すると棋譜を再現できます。"
    );
  }
}
...
function prepareNewReview() {
  ...
  activeReplaySource = null;
  replayView.showUnavailable(
    "新しい振り返りです。KIFを読み込むか棋譜本文を入力してから再現してください。"
  );
}
...
function editReview(reviewId) {
  ...
  formView.showFeedback({
    ...
    message: `${reviewId}を入力フォームへ読み込みました。同じ振り返りIDの更新として保存されます。`
  });
...
function showMarkdownArtifact(artifact) {
  ...
  formView.showFeedback({
    ...
    message: `${artifact.fileName}を確認しています。コピーまたは書き出してObsidianへ登録できます。`
  });
}
...
function deleteReview(reviewId) {
  const accepted = window.confirm("この振り返りを削除します。ブラウザ保存まで成功した場合だけ削除を確定します。続けますか？");
  if (!accepted) return;
  ...
  refreshLibrary();
  formView.showFeedback({ kind: "success", title: "振り返りを削除しました", message: `${reviewId}を保存済み対局とブラウザ保存データから削除しました。` });
}
...
try {
  ...
  if (restored.status === "RESTORED_FROM_BROWSER") {
    formView.showFeedback({ kind: "success", title: "ブラウザ保存データを読み込みました", message: `${restored.count}件の振り返りを復元しました。` });
  }
} catch (error) {
  refreshLibrary();
...
document.getElementById("clear-kif-paste").addEventListener("click", () => {
  kifImportDraftResetController.clearInput();
});

document.getElementById("cancel-kif-import").addEventListener("click", () => {
  kifImportDraftResetController.retryInput();
});
...
document.getElementById("replay-move-list").addEventListener("click", (event) => {
  ...
  const rendered = replayController.jump(Number(moveButton.dataset.jump));
  if (rendered) replayView.scrollIntoView({ behavior: "smooth" });
});
  ...
  try {
    ...
    if (
      ...
      formView.showFeedback({
        ...
        message: "現在局面は入力フォームの候補へ追加されますが、保存ボタンを押すまで保存済みデータは変更されません。",
        focus: false
      });
    }
    ...
    if (result.status === "REJECTED") {
      ...
      formView.showFeedback({
        ...
        focus: result.duplicateIndex !== undefined
      });
    ...
    formView.showFeedback({
      ...
      message: "手数・指し手・局面記録だけを反映しました。事実（FACT）・解釈（INTERPRETATION）・仮説（HYPOTHESIS）は自分の言葉で入力してください。",
      details: result.hasWarning ? ["棋譜再現時の注意事項を局面記録へ保持しています。"] : [],
      focus: false
    });
  } catch (error) {
    showUnexpected(error);
...
document.getElementById("game-review-form").addEventListener("submit", (event) => {
  ...
  formView.showFeedback({
    ...
    message: result.gameReview.readyForNextGame
      ? "次局へ接続できる条件を満たしています。保存済み対局の詳細から次局用Observation Cardを作成できます。"
      : "振り返りデータは保存済みですが、次局へ接続するための項目が残っています。",
    details: missing
  });
});
...
document.getElementById("copy-markdown").addEventListener("click", async () => {
  try {
    ...
    formView.showFeedback({ kind: "success", title: "Markdownをコピーしました", message: `${result.fileName}の内容をクリップボードへコピーしました。` });
...
document.getElementById("download-markdown").addEventListener("click", () => {
  try {
    ...
    formView.showFeedback({ kind: "success", title: ".mdファイルを書き出しました", message: `${result.fileName}をObsidian保管庫へ移動できます。` });
  } catch (error) { showUnexpected(error); }
});
...
document.getElementById("save-browser").addEventListener("click", () => {
  try {
    ...
    formView.showFeedback({ kind: "success", title: "ブラウザへ保存しました", message: `${result.byteLength}バイトを保存しました。` });
  } catch (error) { showUnexpected(error); }
});
...
document.getElementById("load-browser").addEventListener("click", () => {
  try {
    ...
    formView.showFeedback({
      ...
      title: result.status === "EMPTY" ? "保存データがありません" : "ブラウザ保存データを読み込みました",
...
document.getElementById("download-backup").addEventListener("click", () => {
  try {
    ...
    formView.showFeedback({ kind: "success", title: "JSONバックアップを作成しました", message: `${backup.gameReviewCount}件を出力しました。` });
  } catch (error) { showUnexpected(error); }
});
...
document.getElementById("restore-backup").addEventListener("change", async (event) => {
  ...
  try {
    ...
    markdownExportView.clear();
    refreshLibrary();
    formView.showFeedback({ kind: "success", title: "JSONバックアップを復元しました", message: `${result.count}件をまとめて安全に復元しました。` });
  } catch (error) {
    showUnexpected(error);
  } finally {
    event.target.value = "";
...
document.getElementById("delete-browser").addEventListener("click", () => {
  const accepted = window.confirm("ブラウザ保存データだけを削除します。現在メモリ上の振り返りは削除されません。続けますか？");
  if (!accepted) return;
  try {
    backupController.deleteBrowserSavedData();
    formView.showFeedback({ kind: "warning", title: "ブラウザ保存データを削除しました", message: "現在メモリ上のデータは維持されています。" });
  } catch (error) { showUnexpected(error); }
});
